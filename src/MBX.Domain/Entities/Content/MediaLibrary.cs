...
public class MediaLibrary : BaseEntity
{
    [Required]
    [MaxLength(50)]
    public string MediaType { get; set; } = string.Empty; // Loại media (ví dụ: "Image", "Video", "Document") (bắt buộc, độ dài tối đa 50 ký tự)

    [Required]
    [MaxLength(255)]
    public string FileName { get; set; } = string.Empty; // Tên file (bắt buộc, độ dài tối đa 255 ký tự)

    [Required]
    [MaxLength(255)]
    public string FilePath { get; set; } = string.Empty; // Đường dẫn file (bắt buộc, độ dài tối đa 255 ký tự, có thể là đường dẫn tương đối hoặc tuyệt đối)
