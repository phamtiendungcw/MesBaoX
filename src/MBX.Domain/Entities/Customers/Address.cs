...
public class Address : BaseEntity
{
    ...
    [Required]
    [MaxLength(255)]
    public string AddressLine1 { get; set; } = string.Empty; // Địa chỉ dòng 1 (bắt buộc, độ dài tối đa 255 ký tự)

    [MaxLength(255)]
    public string? AddressLine2 { get; set; } // Địa chỉ dòng 2 (không bắt buộc, độ dài tối đa 255 ký tự)

    [Required]
    [MaxLength(100)]
    public string City { get; set; } = string.Empty; // Thành phố (bắt buộc, độ dài tối đa 100 ký tự)

    [Required]
    [MaxLength(100)]
    public string StateProvince { get; set; } = string.Empty; // Tỉnh/Thành phố trực thuộc TW (bắt buộc, độ dài tối đa 100 ký tự)

    [MaxLength(20)]
    public string? ZipCode { get; set; } // Mã bưu điện (không bắt buộc, độ dài tối đa 20 ký tự)

    [Required]
    [MaxLength(100)]
    public string Country { get; set; } = string.Empty; // Quốc gia (bắt buộc, độ dài tối đa 100 ký tự)
