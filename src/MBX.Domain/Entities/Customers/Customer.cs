...
{
    [Required][MaxLength(255)] public string FirstName { get; set; } = string.Empty; // Tên khách hàng (bắt buộc, độ dài tối đa 255 ký tự)
    [Required][MaxLength(255)] public string LastName { get; set; } = string.Empty; // Họ khách hàng (bắt buộc, độ dài tối đa 255 ký tự)
    ...
    [MaxLength(20)] public string? PhoneNumber { get; set; } // Số điện thoại khách hàng (không bắt buộc, độ dài tối đa 20 ký tự)
    ...
    [MaxLength(20)] public string? Gender { get; set; } // Giới tính (không bắt buộc, độ dài tối đa 20 ký tự)
