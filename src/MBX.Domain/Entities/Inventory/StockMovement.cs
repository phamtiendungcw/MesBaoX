...
{
    [Required] public Guid ProductId { get; set; } // ID sản phẩm (khóa ngoại đến bảng Product, bắt buộc)
    ...
    [Required] public Guid WarehouseId { get; set; } // ID kho hàng (khóa ngoại đến bảng Warehouse, bắt buộc)
    public virtual Warehouse Warehouse { get; set; } = null!; // Navigation property cho kho hàng (bắt buộc)
    [Required][MaxLength(50)] public string MovementType { get; set; } = string.Empty; // Loại nhập/xuất kho (ví dụ: "Inbound", "Outbound", "Adjustment") (bắt buộc, độ dài tối đa 50 ký tự)
    ...
    [MaxLength(255)] public string? ReferenceDocument { get; set; } // Tài liệu tham khảo (ví dụ: Mã phiếu nhập kho, Mã đơn hàng) (không bắt buộc, độ dài tối đa 255 ký tự)
    [MaxLength(4000)] public string? Description { get; set; } // Mô tả nhập/xuất kho (không bắt buộc, độ dài tối đa 4000 ký tự)
}