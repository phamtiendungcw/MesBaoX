...
{
    [Required]
    public Guid OrderReturnId { get; set; } // ID yêu cầu trả hàng (khóa ngoại đến bảng OrderReturn, bắt buộc)

    public virtual OrderReturn OrderReturn { get; set; } = null!; // Navigation property cho yêu cầu trả hàng (bắt buộc)

    [Required]
    public Guid OrderItemId { get; set; } // ID chi tiết đơn hàng gốc (khóa ngoại đến bảng OrderItem, bắt buộc)
