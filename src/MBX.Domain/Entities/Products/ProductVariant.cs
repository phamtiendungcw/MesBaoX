...
{
    [Required] public Guid ProductId { get; set; } // ID sản phẩm gốc (khóa ngoại đến bảng Product, bắt buộc)
    public virtual Product Product { get; set; } = null!; // Navigation property cho sản phẩm gốc (bắt buộc)
    [Required][MaxLength(50)] public string SKU { get; set; } = string.Empty; // Mã SKU biến thể (bắt buộc, độ dài tối đa 50 ký tự, duy nhất trong biến thể sản phẩm)
    [Column(TypeName = "decimal(18, 2)")] public decimal Price { get; set; } // Giá biến thể (bắt buộc)
    [Column(TypeName = "decimal(18, 2)")] public decimal? SalePrice { get; set; } // Giá khuyến mãi biến thể (không bắt buộc)
    public int QuantityInStock { get; set; } // Số lượng tồn kho biến thể (bắt buộc)
    [MaxLength(255)] public string? VariantName { get; set; } // Tên biến thể (không bắt buộc, có thể tự động tạo từ thuộc tính)
    [MaxLength(255)] public string? ImageUrl { get; set; } // URL hình ảnh biến thể (không bắt buộc, nếu có ảnh riêng cho biến thể)
