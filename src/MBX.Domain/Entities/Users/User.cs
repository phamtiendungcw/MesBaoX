...
public class User : BaseEntity
{
    [Required]
    [MaxLength(255)]
    public string Username { get; set; } = string.Empty; // Tên đăng nhập (bắt buộc, độ dài tối đa 255 ký tự, duy nhất trong hệ thống)
    ...
    [Required]
    [MaxLength(255)]
    public string FirstName { get; set; } = string.Empty; // Tên người dùng (bắt buộc, độ dài tối đa 255 ký tự)

    [Required]
    [MaxLength(255)]
    public string LastName { get; set; } = string.Empty; // Họ người dùng (bắt buộc, độ dài tối đa 255 ký tự)
